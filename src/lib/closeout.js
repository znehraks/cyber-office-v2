...
function assertPriorityCoverage(backlog) {
  for (const item of backlog) {
    if (!["P0", "P1", "P2", "P3"].includes(item.priority)) {
      throw new Error(`Unclassified backlog item: ${item.id ?? item.title ?? "unknown"}`);
    }
    if (["P0", "P1"].includes(item.priority) && item.status !== "done") {
      throw new Error(`Mission still has unresolved ${item.priority} backlog: ${item.title}`);
    }
  }
}
...
function assertHeaders(fileName, content) {
...
async function assertRequiredReports(root, mission) {
  for (const reportKey of mission.closeout?.required_reports ?? []) {
    ...
    for (const file of files) {
      ...
      if (report?.mission_id === mission.mission_id && report.report_key === reportKey) {
        match.push(report);
      }
    }
    if (match.length === 0) {
...
export async function verifyMissionCloseout(root, missionId) {
  ...
  const nextStepsContent = await requireFile(path.join(artifactDir, "NEXT-STEPS.md"));
  const closeout = await readJson(path.join(artifactDir, "closeout.json"));
