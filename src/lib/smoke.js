...
async function ensureSmokeInput(root) {
...
function buildPacket(root, jobId, inputRef) {
  return {
    ...
    optional_refs: [],
    code_refs: [path.join(root, "src", "cli.js")],
...
async function writeCloseoutArtifacts(root, missionId) {
...
export async function runSmokeScenario(root, options = {}) {
  const inputRef = await ensureSmokeInput(root);
  const messageId = options.messageId ?? `smoke-${Date.now()}`;
