...
export function jobFile(root, jobId) {
  return runtimePath(root, "jobs", `${jobId}.json`);
}

export async function readJob(root, jobId) {
  return readJson(jobFile(root, jobId));
}

export async function writeJob(root, job) {
  await writeJson(jobFile(root, job.job_id), job);
  return job;
}

function computeJobKey(spec) {
  return spec.jobKey ?? `${spec.missionId}:${spec.worker}:${spec.category}:${spec.task}:${spec.attemptNo ?? 0}`;
}

async function createUniqueIndex(indexPath, value) {
  try {
    ...
    if (error && error.code === "EEXIST") {
      return { created: false, value: await readJson(indexPath) };
    }
    throw error;
  }
}

export async function createJob(root, spec) {
  ...
  const jobId = attemptNo === 0 ? baseJobId : `${baseJobId}--attempt-${attemptNo}`;

  const indexed = await createUniqueIndex(indexPath, { job_id: jobId, job_key: jobKey });
  if (!indexed.created) {
    return readJob(root, indexed.value.job_id);
  }
...
export async function transitionJobStatus(root, jobId, fromStatuses, toStatus, patch = {}, options = {}) {
...
export async function updateJob(root, jobId, updater, options = {}) {
  return withResourceLock(root, `job-${jobId}`, async () => {
    const job = await readJob(root, jobId);
    if (!job) {
      throw new Error(`Job not found: ${jobId}`);
...
export async function touchJobActivity(root, jobId, options = {}) {
  return updateJob(
    root,
    jobId,
    async (job) => {
...
export async function readPacket(root, jobId) {
  return readJson(runtimePath(root, "packets", jobId, "manifest.json"));
}

export async function ensureRetryJob(root, originalJob, options = {}) {
...
export async function assertPacketRefs(root, jobId) {
