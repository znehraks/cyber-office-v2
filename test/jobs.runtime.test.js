...
import { createJob, readJob, transitionJobStatus, writePacket } from "../src/lib/jobs.js";
...
async function seedMission(root, missionId = "mission-jobs-1") {
...
test("createJob is idempotent for the same job_key", async () => {
  ...
  const jobs = (await fs.readdir(path.join(root, "runtime", "jobs"))).filter((file) => file.endsWith(".json"));
...
test("transitionJobStatus rejects invalid compare-and-swap transitions", async () => {
  ...
  const first = await transitionJobStatus(root, job.job_id, ["queued"], "running");
  assert.equal(first.changed, true);
  assert.equal(first.job.status, "running");

  const second = await transitionJobStatus(root, job.job_id, ["queued"], "completed");
